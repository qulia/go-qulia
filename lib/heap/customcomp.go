...
type Flex[T lib.Lesser[T]] struct {
	maxOnTop bool
	buffer   []T
}

func newFlex[T lib.Lesser[T]](input []T, maxOnTop bool) *Flex[T] {
	buffer := make([]T, len(input))
	copy(buffer, input)
	return initHeap(buffer, maxOnTop)
}

func initHeap[T lib.Lesser[T]](buffer []T, maxOnTop bool) *Flex[T] {
	h := Flex[T]{buffer: buffer, maxOnTop: maxOnTop}
	h.heapify()
	return &h
}

func (h *Flex[T]) Insert(elem T) {
	// Insert at the end, sift up
	h.buffer = append(h.buffer, elem)
	h.siftUp(h.Size() - 1)
}

func (h *Flex[T]) Extract() T {
...
func (h Flex[T]) IsEmpty() bool {
	return h.Size() == 0
}

func (h Flex[T]) Size() int {
	return len(h.buffer)
}

func (h *Flex[T]) siftUp(index int) {
...
func (h *Flex[T]) siftDown(index int) {
	// If at the leaf, done
	if index >= h.Size()/2 {
		return
	}
...
func (h *Flex[T]) swap(i, j int) {
	h.buffer[i], h.buffer[j] = h.buffer[j], h.buffer[i]
}

func (h *Flex[T]) heapify() {
	if h.Size() <= 1 {
		return
	}
...
func (h *Flex[T]) getTop(indices []int) int {
...
func (h *Flex[T]) less(one, other T) bool {
	if h.maxOnTop {
